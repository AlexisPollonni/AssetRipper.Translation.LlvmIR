...
internal static class BinaryMathInstruction
{
	public static Instruction Create(LLVMValueRef instruction, ModuleContext Module)
	{
		...
		{
			TypeDefinition type =
				resultTypeSignature.Resolve() ?? throw new NullReferenceException(nameof(type));
			...
			{
				MethodDefinition method = Module.InlineArrayNumericHelperType.Methods.First(m =>
					m.Name == methodName
				);
				...
				IMethodDescriptor methodDescriptor = method.MakeGenericInstanceMethod(
					resultTypeSignature,
					typeParameter
				);
			...
			{
				MethodDefinition method = Module.NumericHelperType.Methods.First(m =>
					m.Name == methodName
				);
		...
		else
		{
			throw new NotSupportedException(
				$"Unsupported result type signature: {resultTypeSignature}"
			);
		}
	}
	...
	public static bool Supported(LLVMOpcode opcode) =>
		...
		{
			LLVMOpcode.LLVMAdd
			or LLVMOpcode.LLVMFAdd
			or LLVMOpcode.LLVMSub
			or LLVMOpcode.LLVMFSub
			or LLVMOpcode.LLVMMul
			or LLVMOpcode.LLVMFMul
			or LLVMOpcode.LLVMSDiv
			or LLVMOpcode.LLVMFDiv
			or LLVMOpcode.LLVMSRem
			or LLVMOpcode.LLVMFRem
			or LLVMOpcode.LLVMUDiv
			or LLVMOpcode.LLVMURem
			or LLVMOpcode.LLVMShl
			or LLVMOpcode.LLVMLShr
			or LLVMOpcode.LLVMAShr
			or LLVMOpcode.LLVMAnd
			or LLVMOpcode.LLVMOr
			or LLVMOpcode.LLVMXor => true,
			_ => false,
		};

	private static CilOpCode GetCilOpCode(
		LLVMOpcode npcode,
		bool noSignedWrap,
		bool noUnsignedWrap
	) =>
		npcode switch
		{
			LLVMOpcode.LLVMAdd => noSignedWrap ? CilOpCodes.Add_Ovf
			: noUnsignedWrap ? CilOpCodes.Add_Ovf_Un
			: CilOpCodes.Add,
			LLVMOpcode.LLVMFAdd => CilOpCodes.Add,
			LLVMOpcode.LLVMSub => noSignedWrap ? CilOpCodes.Sub_Ovf
			: noUnsignedWrap ? CilOpCodes.Sub_Ovf_Un
			: CilOpCodes.Sub,
			LLVMOpcode.LLVMFSub => CilOpCodes.Sub,
			LLVMOpcode.LLVMMul => noSignedWrap ? CilOpCodes.Mul_Ovf
			: noUnsignedWrap ? CilOpCodes.Mul_Ovf_Un
			: CilOpCodes.Mul,
	...
	private static string GetName(LLVMOpcode opcode, bool noSignedWrap, bool noUnsignedWrap) =>
		opcode switch
		{
			LLVMOpcode.LLVMAdd => noSignedWrap ? nameof(NumericHelper.AddSigned)
			: noUnsignedWrap ? nameof(NumericHelper.AddUnsigned)
			: nameof(NumericHelper.Add),
			LLVMOpcode.LLVMFAdd => nameof(NumericHelper.Add),
			LLVMOpcode.LLVMSub => noSignedWrap ? nameof(NumericHelper.SubtractSigned)
			: noUnsignedWrap ? nameof(NumericHelper.SubtractUnsigned)
			: nameof(NumericHelper.Subtract),
			LLVMOpcode.LLVMFSub => nameof(NumericHelper.Subtract),
			LLVMOpcode.LLVMMul => noSignedWrap ? nameof(NumericHelper.MultiplySigned)
			: noUnsignedWrap ? nameof(NumericHelper.MultiplyUnsigned)
