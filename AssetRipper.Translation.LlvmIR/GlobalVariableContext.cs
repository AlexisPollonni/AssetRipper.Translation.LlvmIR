...
internal sealed class GlobalVariableContext : IHasName, IVariable
{
	...
	public TypeSignature PointerType =>
		PointerField?.Signature?.FieldType ?? DataGetMethod.Signature!.ReturnType.MakePointerType();
	...
	public void CreateProperties()
	{
		...
		// Pointer field
		{
			PointerField = new FieldDefinition(
				"__pointer",
	...
	public void InitializeData()
	{
		...
		// Initialize pointer field
		{
			...
			instructions.Add(
				CilOpCodes.Call,
				Module
					.InjectedTypes[typeof(NativeMemoryHelper)]
					.Methods.First(m =>
					{
						return m.Name == nameof(NativeMemoryHelper.Allocate)
							&& m.Parameters[0].ParameterType.ElementType is ElementType.I4;
					})
			);
			instructions.Add(
				CilOpCodes.Call,
	...
	public void RemovePointerFieldIfNotUsed()
	{
		if (!PointerIsUsed)
		{
			...
			DeclaringType
				...
				.CilMethodBody!.Instructions.RemoveRange(0, PointerInstructionsCount);
			PointerInstructionsCount = 0;

			FieldDefinition backingField = new(
				"__value",
				FieldAttributes.Private | FieldAttributes.Static,
				DataGetMethod.Signature!.ReturnType
			);
