...
internal sealed class EnumContext : IHasName
{
	...
	private EnumContext(ModuleContext module, TypeDefinition definition, LLVMMetadataRef metadata)
	{
		...
		if (string.IsNullOrEmpty(name) && !metadata.IdentifierDemangled.Contains(">:"))
		{
			...
			name =
				index >= 0 ? $"{cleanIdentifier.AsSpan(0, index)}_unnamed_enum" : cleanIdentifier;
		}
		CleanName = module.Options.StripNamePrefix(NameGenerator.CleanName(name, "Enum"));
	}

	public static EnumContext Create(ModuleContext module, LLVMMetadataRef metadata)
	{
		if (
			metadata.Kind != LLVMMetadataKind.LLVMDICompositeTypeMetadataKind
			|| metadata.TagString is not "DW_TAG_enumeration_type"
		)
		{
		...
		ElementType elementType = metadata.SizeInBits switch
		{
			...
			_ => throw new NotSupportedException(
				$"Enum with size {metadata.SizeInBits} bits is not supported."
			),
		};

		{
			FieldDefinition field = new(
				...
				FieldAttributes.Public
					...
					| FieldAttributes.RuntimeSpecialName,
				module.Definition.CorLibTypeFactory.FromElementType(elementType)
			);
