...
internal static partial class NumericHelper
{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static T FShL<T>(T x, T y, T z) where T : unmanaged, IShiftOperators<T, int, T>, IBitwiseOperators<T, T, T>
	{
		int num;
		unchecked
		{
			num = ConvertToInt32(z) % checked(Unsafe.SizeOf<T>() * 8);
		}
		int num2 = Unsafe.SizeOf<T>() * 8 - num;
		return (x << num) | (y >>> num2);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static T FShR<T>(T x, T y, T z) where T : unmanaged, IShiftOperators<T, int, T>, IBitwiseOperators<T, T, T>
	{
		int num;
		unchecked
		{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOeq<T>(T x, T y) where T : IEqualityOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x == y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOne<T>(T x, T y) where T : IEqualityOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x != y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOgt<T>(T x, T y) where T : IComparisonOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x > y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOge<T>(T x, T y) where T : IComparisonOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x >= y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOlt<T>(T x, T y) where T : IComparisonOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x < y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool FcmpOle<T>(T x, T y) where T : IComparisonOperators<T, T, bool>, INumberBase<T>
	{
		return !IsUnordered(x, y) && x <= y;
	}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpEq<T>(T x, T y) where T : IEqualityOperators<T, T, bool>
	{
		return x == y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpNe<T>(T x, T y) where T : IEqualityOperators<T, T, bool>
	{
		return x != y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpSgt<T>(T x, T y) where T : IComparisonOperators<T, T, bool>
	{
		return x > y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpSge<T>(T x, T y) where T : IComparisonOperators<T, T, bool>
	{
		return x >= y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpSlt<T>(T x, T y) where T : IComparisonOperators<T, T, bool>
	{
		return x < y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpSle<T>(T x, T y) where T : IComparisonOperators<T, T, bool>
	{
		return x <= y;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpUgt<T>(T x, T y) where T : struct
	{
		return Unsafe.BitCast<T, UInt128>(x) > Unsafe.BitCast<T, UInt128>(y);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpUge<T>(T x, T y) where T : struct
	{
		return Unsafe.BitCast<T, UInt128>(x) >= Unsafe.BitCast<T, UInt128>(y);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpUlt<T>(T x, T y) where T : struct
	{
		return Unsafe.BitCast<T, UInt128>(x) < Unsafe.BitCast<T, UInt128>(y);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool IntCmpUle<T>(T x, T y) where T : struct
	{
		return Unsafe.BitCast<T, UInt128>(x) <= Unsafe.BitCast<T, UInt128>(y);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public unsafe static TTarget TruncOrZextToBytes<TSource, TTarget>(TSource source) where TSource : struct where TTarget : struct
	{
		TTarget value = default(TTarget);
		Buffer.MemoryCopy(sourceBytesToCopy: (uint)Math.Min(Unsafe.SizeOf<TSource>(), Unsafe.SizeOf<TTarget>()), source: Unsafe.AsPointer(in source), destination: Unsafe.AsPointer(in value), destinationSizeInBytes: Unsafe.SizeOf<TTarget>());
		return value;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public unsafe static TTarget SextToBytes<TSource, TTarget>(TSource source, int sourceBits) where TSource : struct where TTarget : struct
	{
		TTarget value = default(TTarget);
		int num = Unsafe.SizeOf<TSource>();
		Buffer.MemoryCopy(Unsafe.AsPointer(in source), Unsafe.AsPointer(in value), Unsafe.SizeOf<TTarget>(), num);
		unchecked
		{
			if ((((byte*)Unsafe.AsPointer(in source))[checked(sourceBits - 1) / 8] & (1 << (checked(sourceBits - 1) & 7))) != 0)
			{
				byte* ptr = (byte*)Unsafe.AsPointer(in value);
				for (int i = num; i < Unsafe.SizeOf<TTarget>(); i = checked(i + 1))
				{
					ptr[i] = byte.MaxValue;
				}
				int num2 = sourceBits & 7;
				if (num2 != 0)
				{
					byte* num3 = ptr + checked(sourceBits - 1) / 8;
					*num3 = checked((byte)(*num3 | (byte)(255 << num2)));
				}
			}
			return value;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static T CtTz<T>(T x) where T : IBinaryInteger<T>
	{
