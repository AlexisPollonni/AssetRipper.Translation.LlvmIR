...
internal static partial class InlineArrayNumericHelper
{
	...
	public static TBuffer FShL<TBuffer, TElement>(TBuffer x, TBuffer y, TBuffer z) where TBuffer : struct, IInlineArray<TElement> where TElement : unmanaged, IShiftOperators<TElement, int, TElement>, IBitwiseOperators<TElement, TElement, TElement>
	{
		TBuffer buffer = default(TBuffer);
		for (int i = 0; i < TBuffer.Length; i++)
		{
			InlineArrayHelper.SetElement(ref buffer, i, NumericHelper.FShL(x.GetElement<TBuffer, TElement>(i), y.GetElement<TBuffer, TElement>(i), z.GetElement<TBuffer, TElement>(i)));
		}
		return buffer;
	}

	public static TBuffer FShR<TBuffer, TElement>(TBuffer x, TBuffer y, TBuffer z) where TBuffer : struct, IInlineArray<TElement> where TElement : unmanaged, IShiftOperators<TElement, int, TElement>, IBitwiseOperators<TElement, TElement, TElement>
	{
		TBuffer buffer = default(TBuffer);
		for (int i = 0; i < TBuffer.Length; i++)
		{
			InlineArrayHelper.SetElement(ref buffer, i, NumericHelper.FShR(x.GetElement<TBuffer, TElement>(i), y.GetElement<TBuffer, TElement>(i), z.GetElement<TBuffer, TElement>(i)));
		}
		return buffer;
	}
	...
	public static TBuffer CtTz<TBuffer, TElement>(TBuffer x) where TBuffer : struct, IInlineArray<TElement> where TElement : IBinaryInteger<TElement>
	{
		TBuffer buffer = default(TBuffer);
		TensorPrimitives.TrailingZeroCount(x.AsReadOnlySpan<TBuffer, TElement>(), buffer.AsSpan<TBuffer, TElement>());
		return buffer;
	}
