...
internal static partial class fputil_NormalFloat_double_cmp_fputil_NormalFloat_double_const_const
{
	public unsafe static int Invoke(fputil_NormalFloat_n2h9ta* @this, fputil_NormalFloat_n2h9ta* other)
	{
		...
		int result = ((!operator_Sign_Sign_rpctvi.Invoke(anon_izyfb8.val, anon_izyfb9.val)) ? ((@this->exponent <= other->exponent) ? ((@this->exponent != other->exponent) ? (-num) : (unchecked((ulong)@this->mantissa <= (ulong)other->mantissa) ? ((@this->mantissa != other->mantissa) ? (-num) : 0) : num)) : num) : num);
