...
{
	public unsafe static int Invoke(printf_core_PaddingWriter* @this, printf_core_Writer* writer, long total_digits)
	{
		int result = 0;
		int num = 0;
		int num2 = 0;
		...
		if ((@this->left_justified & 1) == 1 || num < 0)
		{
			if (@this->sign_char > 0)
			{
				...
				num2 = printf_core_Writer_write_char.Invoke(writer, @this->sign_char);
				int num6;
				if (num2 < 0)
				{
					result = num2;
		...
		else
		{
			...
			if (@this->sign_char > 0)
			{
				...
				num4 = printf_core_Writer_write_char.Invoke(writer, @this->sign_char);
				int num6;
				if (num4 < 0)
				{
					result = num4;
