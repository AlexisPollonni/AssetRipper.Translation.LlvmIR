...
internal static partial class NormalFloat_Float16_cmp
{
	...
	public unsafe static int Invoke([MangledName("this")] Fputil_NormalFloat_epbrnk* This, [MangledName("other")][NativeType("__llvm_libc_20_1_2_::fputil::NormalFloat<_Float16> const&")] Fputil_NormalFloat_epbrnk* Other)
	{
		int num = 0;
		...
		int result = ((!NotEquals_rpctvi.Invoke(anon_izyfb.Val, anon_izyfb2.Val)) ? ((This->Exponent <= Other->Exponent) ? ((This->Exponent != Other->Exponent) ? (-num) : (unchecked((ushort)This->Mantissa <= (ushort)Other->Mantissa) ? (unchecked((ushort)This->Mantissa != (ushort)Other->Mantissa) ? (-num) : 0) : num)) : num) : num);
