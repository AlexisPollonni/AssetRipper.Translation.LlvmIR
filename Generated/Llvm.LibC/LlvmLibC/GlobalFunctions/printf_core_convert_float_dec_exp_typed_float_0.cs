	...
	public unsafe static int Invoke([NativeType("__llvm_libc_20_1_2_::printf_core::Writer*")] printf_core_Writer* writer, [NativeType("__llvm_libc_20_1_2_::printf_core::FormatSection const&")] printf_core_FormatSection* to_conv, [MangledName("float_bits.coerce")][NativeType("__llvm_libc_20_1_2_::fputil::FPBits<float>")] int float_bits)
	{
		...
		unchecked
		{
			...
			*(Int128*)((byte*)(&floatToString_vnf) + 16) = -6148914691236517206L;
			...
			long num20 = IntegerToString_long_details_Fmt_unsigned_char_10_false_false_false_1ul_size.Invoke(&integerToString_uuwdyc);
		...
		long num21 = IntegerToString_long_details_Fmt_unsigned_char_10_false_false_false_1ul_size.Invoke(&integerToString_uuwdyc2);
		llvm_lifetime_end_p0.Invoke(32L, &integerToString_uuwdyc2);
		num10 = num21;
		unchecked
		{
			FloatWriter_init.Invoke(&printf_core_FloatWriter2, (uint)(checked(1 + InstructionHelper.Select((b & 1) == 1, 1, 0)) + num3 + 2) + (((ulong)num10 >= 2uL) ? num10 : 2L), 1L);
			if ((ulong)num8 <= (ulong)(uint)(num3 + 1))
			{
				FloatWriter_write_first_block.Invoke(&printf_core_FloatWriter2, num7, exp_format: true);
				num4 += num8;
				num6 = checked(num6 + -1);
			}
			while (true)
			{
				...
				if ((ulong)(num4 + 9L) < (ulong)(uint)(num3 + 1))
				{
					...
					num11 = FloatWriter_write_middle_block.Invoke(&printf_core_FloatWriter2, num7);
					if (num11 < 0)
					{
						result = num11;
						num22 = 1;
				...
				if (num13 == num12)
				{
					...
					num15 = FloatToString_float_0_get_block.Invoke(&floatToString_vnf, num6);
				...
				if ((b2 & 1) != 1)
				{
					checked
					{
						if (num6 >= 0)
						{
							...
							for (j = num6 - 1; j >= 0; j += -1)
							{
								...
								num17 = FloatToString_float_0_get_block.Invoke(&floatToString_vnf, j);
								if (num17 != 0)
								{
									b2 = 1;
									num22 = 10;
					...
					{
						b2 = (sbyte)(byte)((printf_core_zero_after_digits_unsigned_int.Invoke(checked(num2 - 23), num3 - num5, FPRepSem_fputil_FPType_1_fputil_FPBits_float_get_explicit_mantissa.Invoke(&fputil_FPBits_5nkvcs2), 23) ? 1 : 0) ^ -1);
					}
				}
				int last_digit = num14;
				bool truncated = (b2 & 1) == 1;
				anon_izyfb8.val = FPStorage_fputil_FPType_1_sign.Invoke(&fputil_FPBits_5nkvcs2);
				num16 = (int)printf_core_get_round_direction.Invoke(last_digit, truncated, anon_izyfb8.val);
				llvm_lifetime_start_p0.Invoke(4L, &num18);
				num18 = FloatWriter_write_last_block.Invoke(&printf_core_FloatWriter2, num7, num13, (RoundDirection_zvveyk)num16, num5, InstructionHelper.Select(internal_islower_38ezuj.Invoke(to_conv->conv_name), (sbyte)101, (sbyte)69));
				if (num18 < 0)
				{
					result = num18;
					num22 = 1;
				...
				case 0:
					...
					num19 = FloatWriter_right_pad.Invoke(&printf_core_FloatWriter2);
					if (num19 < 0)
					{
						result = num19;
						num22 = 1;
