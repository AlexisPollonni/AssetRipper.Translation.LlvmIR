	...
	public unsafe static int Invoke(scanf_core_Reader* reader, scanf_core_FormatSection* to_conv)
	{
		...
		unchecked
		{
			...
			else
			{
				if ((b6 & 1) == 1)
				{
					scanf_core_write_int_with_length_unsigned_long_scanf_core_FormatSection_const.Invoke(num7, to_conv);
				}
				else
				{
					if ((b5 & 1) == 1)
					...
					scanf_core_write_int_with_length_unsigned_long_scanf_core_FormatSection_const.Invoke(num2, to_conv);
				}
				result = 0;
			}
