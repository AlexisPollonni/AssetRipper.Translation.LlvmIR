	...
	public unsafe static float Invoke(float x)
	{
		...
		unchecked
		{
			...
			*(sbyte*)(&anon_izyfb8) = -86;
			...
			int num4 = fputil_internal_FPRepImpl_fputil_FPType_1_fputil_FPBits_float_uintval_const.Invoke(&fputil_FPBits_5nkvcs3);
			llvm_lifetime_end_p0.Invoke(4L, &fputil_FPBits_5nkvcs3);
			num = num4;
			float result;
			if (bool_details_expects_bool_condition_bool_bool_bool.Invoke((uint)num >= 1065353216u, expected: false))
			{
			...
			else if (bool_details_expects_bool_condition_bool_bool_bool.Invoke((uint)num <= 1036779520u, expected: false))
			{
				if (bool_details_expects_bool_condition_bool_bool_bool.Invoke((uint)num <= 847249408u, expected: false))
				{
					result = (float)((!bool_details_expects_bool_condition_bool_bool_bool.Invoke(num == 0, expected: false)) ? llvm_fmuladd_f64.Invoke(1.0 / 3.0 * (double)x * (double)x, x, x) : ((double)x));
				}
				else
				{
			...
			else
			{
				...
				num3 = x;
				result = (float)(0.5 * log_eval_double.Invoke((num3 + 1.0) / (num3 - 1.0)));
