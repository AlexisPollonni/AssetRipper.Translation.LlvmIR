using LlvmLibC.GlobalVariables;
using LlvmLibC.Helpers;
using LlvmLibC.Intrinsics.Implemented;
using LlvmLibC.Structures;

namespace LlvmLibC.GlobalFunctions;

[MangledName("_ZZN19__llvm_libc_20_1_2_12__cos_impl__EdENK3$_0clEj")]
[DemangledName("__llvm_libc_20_1_2_::__cos_impl__(double)::$_0::operator()(unsigned int) const")]
internal static partial class cos_impl_double_0_operator_unsigned_int_const
{
	public unsafe static void Invoke([MangledName("agg.result")] fputil_DyadicFloat_kt2kd4* agg_result, void* @this, int kk)
	{
		int num = 0;
		llvm_lifetime_start_p0.Invoke(4L, &num);
		unchecked
		{
			num = (((kk & 0x40) == 0) ? (kk & 0x3F) : (64 - (kk & 0x3F)));
			*(sbyte*)agg_result = -86;
			sbyte* ptr = (sbyte*)agg_result + 1;
			*ptr = -86;
			ptr[1] = -86;
			ptr[2] = -86;
			((int*)agg_result)[1] = -1431655766;
			sbyte* ptr2 = (sbyte*)agg_result + 8;
			*(long*)ptr2 = -6148914691236517206L;
			((long*)ptr2)[1] = -6148914691236517206L;
			long num2 = (uint)num;
			llvm_memcpy_p0_p0_i64.Invoke(agg_result, (byte*)SIN_K_PI_OVER_128_F128_57.Pointer + (nint)num2 * sizeof(fputil_DyadicFloat_kt2kd4), 24L, isVolatile: false);
			if ((kk & 0x80) != 0)
			{
				llvm_memcpy_p0_p0_i64.Invoke(&agg_result->sign, Sign_NEG.Pointer, 1L, isVolatile: false);
			}
			llvm_lifetime_end_p0.Invoke(4L, &num);
		}
	}
}
