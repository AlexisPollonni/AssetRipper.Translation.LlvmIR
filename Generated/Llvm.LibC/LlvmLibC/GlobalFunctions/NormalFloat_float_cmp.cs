...
internal static partial class NormalFloat_float_cmp
{
	...
	public unsafe static int Invoke([MangledName("this")] Fputil_NormalFloat_phu2qv* This, [MangledName("other")][NativeType("__llvm_libc_20_1_2_::fputil::NormalFloat<float> const&")] Fputil_NormalFloat_phu2qv* Other)
	{
		int num = 0;
		...
		int result = ((!NotEquals_rpctvi.Invoke(anon_izyfb.Val, anon_izyfb2.Val)) ? ((This->Exponent <= Other->Exponent) ? ((This->Exponent != Other->Exponent) ? (-num) : (unchecked((uint)This->Mantissa <= (uint)Other->Mantissa) ? ((This->Mantissa != Other->Mantissa) ? (-num) : 0) : num)) : num) : num);
