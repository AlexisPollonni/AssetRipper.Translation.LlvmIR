...
internal static partial class NormalFloat_float_cmp
{
	...
	public unsafe static int Invoke(fputil_NormalFloat_phu2qv* @this, [NativeType("__llvm_libc_20_1_2_::fputil::NormalFloat<float> const&")] fputil_NormalFloat_phu2qv* other)
	{
		int num = 0;
		...
		int result = ((!NotEquals_rpctvi.Invoke(anon_izyfb8.val, anon_izyfb9.val)) ? ((@this->exponent <= other->exponent) ? ((@this->exponent != other->exponent) ? (-num) : (unchecked((uint)@this->mantissa <= (uint)other->mantissa) ? ((@this->mantissa != other->mantissa) ? (-num) : 0) : num)) : num) : num);
