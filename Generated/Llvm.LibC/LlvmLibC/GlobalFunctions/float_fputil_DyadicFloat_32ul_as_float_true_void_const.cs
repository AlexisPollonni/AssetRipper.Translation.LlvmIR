using AssetRipper.Translation.LlvmIR.Runtime.Attributes;

namespace LlvmLibC.GlobalFunctions;

[MangledName("_ZNK19__llvm_libc_20_1_2_6fputil11DyadicFloatILm32EE2asIfLb1EvEET_v")]
[DemangledName("float __llvm_libc_20_1_2_::fputil::DyadicFloat<32ul>::as<float, true, void>() const")]
internal static partial class float_fputil_DyadicFloat_32ul_as_float_true_void_const
{
	public unsafe static float Invoke(void* @this)
	{
		return float_fputil_DyadicFloat_32ul_fast_as_float_true_void_const.Invoke(@this);
	}
}
