using System;
using LlvmLibC.Helpers;

namespace LlvmLibC.GlobalFunctions;

[MangledName("_ZN19__llvm_libc_20_1_2_6fputil51round_to_signed_integer_using_current_rounding_modeIDF16_lTnNS_3cpp9enable_ifIXaasr3cppE19is_floating_point_vIT_Esr3cppE13is_integral_vIT0_EEiE4typeELi0EEES5_S4_")]
[DemangledName("long __llvm_libc_20_1_2_::fputil::round_to_signed_integer_using_current_rounding_mode<_Float16, long, 0>(_Float16)")]
internal static partial class long_fputil_round_to_signed_integer_using_current_rounding_mode_Float16_long_0_Float16
{
	public static long Invoke(Half x)
	{
		return long_fputil_internal_rounded_float_to_signed_integer_Float16_long_0_Float16.Invoke(cpp_enable_if_cpp_is_floating_point_v_Float16_Float16_type_fputil_round_using_current_rounding_mode_Float16_Float16.Invoke(x));
	}
}
