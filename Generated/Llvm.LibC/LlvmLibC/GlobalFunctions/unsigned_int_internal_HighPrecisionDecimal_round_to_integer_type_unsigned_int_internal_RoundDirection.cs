...
{
	public unsafe static int Invoke(void* @this, int round)
	{
		int num = 0;
		int i = 0;
		...
		unchecked
		{
			...
			{
				num = num * 10 + (byte)((sbyte*)(&((internal_HighPrecisionDecimal*)@this)->digits))[(uint)i];
			}
			for (; i < ((internal_HighPrecisionDecimal*)@this)->decimal_point; i++)
			{
				num *= 10;
			}
			int result = num + (internal_HighPrecisionDecimal_should_round_up_int_internal_RoundDirection.Invoke(@this, ((internal_HighPrecisionDecimal*)@this)->decimal_point, round) ? 1 : 0);
