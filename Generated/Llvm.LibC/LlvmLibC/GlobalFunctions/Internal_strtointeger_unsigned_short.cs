...
internal static partial class Internal_strtointeger_unsigned_short
{
	...
	public unsafe static Struct_fiz2nb Invoke([MangledName("src")][NativeType("char const*")] void* Src, [MangledName("base")][NativeType("int")] int Base, [MangledName("src_len")][NativeType("unsigned long")] long Src_len)
	{
		...
		unchecked
		{
			if (Src_len == 0L)
			{
				StrToNumResult_unsigned_short_Constructor.Invoke(&strToNumResult_wwcts, 0, 0L, 0);
			}
			else if (num8 < 0 || num8 == 1 || num8 > 36)
			{
				StrToNumResult_unsigned_short_Constructor.Invoke(&strToNumResult_wwcts, 0, 0L, 22);
			}
			else
			{
				...
				if (num3 == 34)
				{
					if ((b4 & 1) == 1 || true)
					{
						StrToNumResult_unsigned_short_Constructor.Invoke(&strToNumResult_wwcts, Integer_impl_unsigned_short_unsigned_short_0_unsigned_short_65535_max.Invoke(), parsed_len, num3);
					}
					else
					{
						...
						long parsed_len2;
						int error;
						StrToNumResult_unsigned_short_Constructor.Invoke(&strToNumResult_wwcts, value, parsed_len2, error);
					}
				}
				else
				{
					StrToNumResult_unsigned_short_Constructor.Invoke(&strToNumResult_wwcts, (short)(((b4 & 1) != 1) ? checked(-unchecked((ushort)num)) : ((ushort)num)), parsed_len, num3);
				}
