...
internal static partial class Printf_core_convert_float_dec_exp_typed_float_0
{
	...
	public unsafe static int Invoke([MangledName("writer")][NativeType("__llvm_libc_20_1_2_::printf_core::Writer*")] Printf_core_Writer* Writer, [MangledName("to_conv")][NativeType("__llvm_libc_20_1_2_::printf_core::FormatSection const&")] Printf_core_FormatSection* To_conv, [MangledName("float_bits.coerce")][NativeType("__llvm_libc_20_1_2_::fputil::FPBits<float>")] int Float_bits)
	{
		...
		unchecked
		{
			...
			init_sign_char = 0;
			if (FPRepImpl_fputil_FPType_1_fputil_FPBits_float_is_neg.Invoke((Fputil_internal_FPRepImpl_vhj4tm*)(&fputil_FPBits_5nkvcs)))
			{
				init_sign_char = 45;
			}
		...
		unchecked
		{
			...
			while (true)
			{
					...
					if ((b2 & 1) != 1)
					{
						if (num6 >= 0)
						{
							...
							for (j = num6 - 1; j >= 0; j += -1)
							{
								...
								num16 = FloatToString_float_0_get_block.Invoke(&floatToString_vnf, j);
								if (num16 != 0)
								{
									b2 = 1;
									num21 = 10;
						...
						if ((b2 & 1) != 1 && num2 - 23 < 0)
						{
							b2 = (InstructionHelper.BooleanXor(Printf_core_zero_after_digits_unsigned_int.Invoke(num2 - 23, unchecked(num3 - num5), FPRepSem_fputil_FPType_1_fputil_FPBits_float_get_explicit_mantissa.Invoke(&fputil_FPBits_5nkvcs), 23), right: true) ? ((sbyte)1) : ((sbyte)0));
						}
					}
					last_digit = num14;
					truncated = (b2 & 1) == 1;
				...
				num17 = FloatWriter_write_last_block.Invoke(&printf_core_FloatWriter, num7, num13, (RoundDirection_zvveyk)round, num5, InstructionHelper.Select(Internal_islower_38ezuj.Invoke(To_conv->Conv_name), (sbyte)101, (sbyte)69));
				if (num17 < 0)
				{
					result = num17;
					num21 = 1;
