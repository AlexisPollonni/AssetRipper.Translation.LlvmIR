...
internal static partial class printf_core_vasprintf_internal
{
	...
	public unsafe static int Invoke([NativeType("char**")] void* ret, [NativeType("char const*")] void* format, [NativeType("__llvm_libc_20_1_2_::internal::ArgList")] void* args)
	{
		...
		WriteBuffer_Constructor.Invoke(&printf_core_WriteBuffer2, &inlineArray200_SByte, 200L, printf_core_resize_overflow_hook.__pointer);
		...
		unchecked
		{
			...
			*ptr = -86;
			...
			num = printf_core_printf_main.Invoke(&printf_core_Writer2, format, (internal_ArgList*)args);
			if (num < 0)
			{
				*(IntPtr*)ret = (nint)0;
				result = -1;
			...
			else
			{
				if (printf_core_WriteBuffer2.buff == &inlineArray200_SByte)
				{
					*(void**)ret = malloc.Invoke(checked(num + 1));
					...
					inline_memcpy.Invoke(*(void**)ret, printf_core_WriteBuffer2.buff, num);
				}
				else
				{
