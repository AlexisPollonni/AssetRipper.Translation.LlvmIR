	...
	public unsafe static Struct_8myw6y Invoke([MangledName("this")] Printf_core_Parser* This, [MangledName("index")][NativeType("unsigned long")] long Index)
	{
		...
		if (Index != 0L && Index != This->Args_index)
		{
			...
			if ((b & 1) != 1)
			{
				...
				Optional_long_Constructor.Invoke(&cpp_optional_d6hvji);
				num2 = 1;
			}
			else
			{
		...
		unchecked
		{
			*(short*)(&printf_core_TypeDesc) = Printf_core_type_desc_from_type_long.Invoke();
			...
			Optional_long_Constructor.Invoke(&cpp_optional_d6hvji, &num);
