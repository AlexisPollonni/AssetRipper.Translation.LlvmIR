...
internal static partial class StrToNumResult_long_internal_strtointeger_long_char_const_int_unsigned_long
{
	public unsafe static void Invoke([MangledName("agg.result")] StrToNumResult_aztiru* agg_result, void* src, int @base, long src_len)
	{
		...
		unchecked
		{
			if (src_len == 0L)
			{
				StrToNumResult_long_StrToNumResult_long_long_int.Invoke(agg_result, 0L, 0L, 0);
			}
			else if (num8 < 0 || num8 == 1 || num8 > 36)
			{
				StrToNumResult_long_StrToNumResult_long_long_int.Invoke(agg_result, 0L, 0L, 22);
			}
			else
			{
				...
				num5 = InstructionHelper.Select((b4 & 1) == 1, cpp_internal_integer_impl_long_9223372036854775808l_9223372036854775807l_max.Invoke(), long.MinValue);
				llvm_lifetime_start_p0.Invoke(8L, &num6);
				num6 = (long)((ulong)num5 / (ulong)num8);
				while ((ulong)num2 < (ulong)src_len && internal_isalnum_int_158.Invoke(((sbyte*)src)[num2]))
				{
					llvm_lifetime_start_p0.Invoke(4L, &num7);
					num7 = internal_b36_char_to_int_int_159.Invoke(((sbyte*)src)[num2]);
					int num9;
					if (num7 >= num8)
					{
						num9 = 3;
				...
				if (num3 == 34)
				{
					if ((b4 & 1) == 1)
					{
						StrToNumResult_long_StrToNumResult_long_long_int.Invoke(agg_result, cpp_internal_integer_impl_long_9223372036854775808l_9223372036854775807l_max.Invoke(), parsed_len, num3);
					}
					else
					{
						StrToNumResult_long_StrToNumResult_long_long_int.Invoke(agg_result, cpp_internal_integer_impl_long_9223372036854775808l_9223372036854775807l_min.Invoke(), parsed_len, num3);
					}
				}
				else
				{
					StrToNumResult_long_StrToNumResult_long_long_int.Invoke(agg_result, ((b4 & 1) != 1) ? (-num) : num, parsed_len, num3);
				}
