...
{
	public unsafe static int Invoke(int last_digit, bool truncated, [MangledName("sign.coerce")] sbyte sign)
	{
		...
		case 0:
			if (last_digit != 5)
			{
				return InstructionHelper.Select(last_digit > 5, 0, 1);
			}
			return InstructionHelper.Select(unchecked((byte)((((b & 1) == 1) ? 1u : 0u) ^ 0xFFFFFFFFu)) != 0, 2, 0);
		case 1024:
