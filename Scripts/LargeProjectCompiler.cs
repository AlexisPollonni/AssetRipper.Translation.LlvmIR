...
args = [@"K:\source\repos\AssetRipper.Translation.LlvmIR\artifacts\obj\AssetRipper.Translation.LlvmIR.Shims.Native\Debug\cmake\compile_commands.json",
...
static int Compile([Argument] string compileCommandsJson, [Argument] string outputDir, string[]? excludePrefixes = null)
{
    ...
    FileExists(compileCommandsJsonPath).ShouldBeTrue("The compile_commands.json file does not exist.");
    ...
    jsonOptions.Converters.AddRange([new FilePathJsonConverter(), new DirectoryPathJsonConverter(), new CommandJsonConverter()]);

#pragma warning disable IL2026, IL3050
    var compileCommands = JsonSerializer.Deserialize<CompileCommand[]>(compileCommandsContent, jsonOptions) ?? [];
#pragma warning restore IL2026, IL3050


    var bcFiles = compileCommands.DistinctBy(c => c.Output)
    ...
    .Where(c =>
    {
        ...
        return compiler.Path.GetFilenameWithoutExtension().ToString().Contains("clang", StringComparison.OrdinalIgnoreCase);
    })
    .Select(c =>
    {
        var transformedOutput = outputDirPath.CombineWithFilePath(c.File.GetFilename().ChangeExtension(".bc"));
        return c with
        {
    ...
    try
    {
        ...
        .ForAll(c =>
        {
            Information($"[{c.Index}/{toCompile.Length}] Compiling {compileCommandsJsonPath.GetRelativePath(c.Item.File)} -> {outputDirPath.GetRelativePath(c.Item.Output)}");
...
static List<string> RewriteCommand(IEnumerable<string> commandParts, FilePath inputPath, FilePath outputPath, FilePath originalOutputPath)
{
    ...
    {
        if (string.Equals(filtered[i], originalOutputPath.FullPath, StringComparison.OrdinalIgnoreCase))
    ...
    var mandatory = new[] { "-w", "-DNOMINMAX", "-g", "-fno-discard-value-names", "-fstandalone-debug" };
...
static unsafe void LinkModulesToFinal(IEnumerable<FilePath> bitcodeFiles, FilePath outputmergedBitcode)
{
    ...
    var modules = files.Zip(buffers).Select((f, i) =>
    {
        var buffer = f.Second;
        ...
        try
        {
            if (!context.TryGetBitcodeModule(buffer, out module, out var errorMessage))
                Warning($"[{i}/{files.Length}] Failed to parse bitcode file '{f.First}': {errorMessage}");
            else
    ...
    modules.Where(m => !m.Module.TryVerify(LLVMVerifierFailureAction.LLVMPrintMessageAction, out _))
        .ToList()
    ...
    foreach (var m in modules)
    {
        Debugger.Launch();
        Debugger.Break();
        Information($"Linking module {m.Path.GetRelativePath(outputmergedBitcode.GetDirectory())} into final module.");
...
{
    return commands.Where(command => !FileExists(command.Output) || System.IO.File.GetLastWriteTimeUtc(command.File.FullPath) > System.IO.File.GetLastWriteTimeUtc(command.Output.FullPath));
}
...
static unsafe void HookLLVMContextCallback(LLVMContextRef context)
{
    ...
    context.SetYieldCallback((LLVMOpaqueContext* ctxPtr, void* _) =>
    {
...
class FilePathJsonConverter : JsonConverter<FilePath>
{
    ...
    {
        var str = reader.GetString() ?? throw new JsonException("Expected a string value for FilePath.");
...
class DirectoryPathJsonConverter : JsonConverter<DirectoryPath>
{
    ...
    {
        var str = reader.GetString() ?? throw new JsonException("Expected a string value for DirectoryPath.");
...
class CommandJsonConverter : JsonConverter<List<string>>
{
    ...
    {
        var str = reader.GetString() ?? throw new JsonException("Expected a string value for Command.");
