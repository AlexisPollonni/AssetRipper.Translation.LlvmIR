...
public partial class NumericGenerator() : IncrementalGenerator(nameof(NumericGenerator))
{
	...
	private static string GenerateNumericHelper()
	{
		...
		writer.WriteUsing("AssetRipper.Translation.LlvmIR");
		writer.WriteLineNoTabs();
		writer.WriteFileScopedNamespace("AssetRipper.Translation.LlvmIR.Runtime");
		writer.WriteLineNoTabs();
		...
		using (new CurlyBrackets(writer))
		{
			...
			foreach (
				(
					...
					string? dotNetName,
					string? requiredInterfaces
				) in TertiaryInterfaceOperations
			)
			{
				writer.WriteLineNoTabs();
				writer.WriteLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
	...
	private static string GenerateInlineArrayNumericHelper()
	{
		...
		using (new CurlyBrackets(writer))
		{
			...
			WriteTertiaryNumericHelperMethod(
				writer,
				"FShL",
				"unmanaged, IShiftOperators<TElement, int, TElement>, IBitwiseOperators<TElement, TElement, TElement>"
			);
			WriteTertiaryNumericHelperMethod(
				writer,
				"FShR",
			...
			{
				WriteTertiaryTensorPrimitivesMethod(
					writer,
					llvmName,
					dotNetName,
					ReplaceTypeParameterName(requiredInterfaces)
		...
		static void WriteTertiaryTensorPrimitivesMethod(
			IndentedTextWriter writer,
			string llvmName,
			string? dotNetName,
			string? requiredInterfaces
		)
		{
			writer.WriteLineNoTabs();
			writer.WriteLine(
				$"public static TBuffer {llvmName}<TBuffer, TElement>(TBuffer x, TBuffer y, TBuffer z)"
			);
			using (new Indented(writer))
			{
				writer.WriteLine("where TBuffer : struct, IInlineArray<TElement>");
			...
			using (new CurlyBrackets(writer))
			{
				writer.WriteLine("TBuffer result = default;");
				writer.WriteLine(
					$"TensorPrimitives.{(string.IsNullOrEmpty(dotNetName) ? llvmName : dotNetName)}(x.AsReadOnlySpan<TBuffer, TElement>(), y.AsReadOnlySpan<TBuffer, TElement>(), z.AsReadOnlySpan<TBuffer, TElement>(), result.AsSpan<TBuffer, TElement>());"
				);
				writer.WriteLine("return result;");
			}
		}

		static void WriteTertiaryNumericHelperMethod(
			IndentedTextWriter writer,
			string methodName,
			string? requiredInterfaces
		)
		{
			writer.WriteLineNoTabs();
			writer.WriteLine(
				$"public static TBuffer {methodName}<TBuffer, TElement>(TBuffer x, TBuffer y, TBuffer z)"
			);
			using (new Indented(writer))
			{
				writer.WriteLine("where TBuffer : struct, IInlineArray<TElement>");
			...
			using (new CurlyBrackets(writer))
			{
				...
				{
					writer.WriteLine(
						$"result.SetElement(i, NumericHelper.{methodName}<TElement>(x.GetElement<TBuffer, TElement>(i), y.GetElement<TBuffer, TElement>(i), z.GetElement<TBuffer, TElement>(i)));"
					);
				}
				writer.WriteLine("return result;");
			}
